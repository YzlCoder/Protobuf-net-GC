...
namespace Sirenix.OdinInspector.Editor
{
    ...
    public static class AtomHandlerLocator
    {
        ...
        {
            var atomHandlers = AppDomain.CurrentDomain.GetAssemblies()
                .Where(ass => ass.IsDefined(typeof(AtomContainerAttribute), true)).SelectMany(ass => ass.GetTypes())
                .Where(
                    type => !type.IsAbstract && type.IsDefined(typeof(AtomHandlerAttribute), false)
                                             && type.GetConstructor(Type.EmptyTypes) != null
                                             && typeof(IAtomHandler).IsAssignableFrom(type)
                                             && type.ImplementsOpenGenericInterface(typeof(IAtomHandler<>)));
            ...
            foreach (var handler in atomHandlers)
            {
                ...
                {
                    Debug.LogError(
                        "The type '" + atomicType.GetNiceName() + "' cannot be marked atomic, as it is abstract.");
                    continue;
                }
