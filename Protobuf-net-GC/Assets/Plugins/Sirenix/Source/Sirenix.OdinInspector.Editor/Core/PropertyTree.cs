...
namespace Sirenix.OdinInspector.Editor
{
    ...
    public abstract class PropertyTree
    {
        ...
        {   
#if UNITY_EDITOR
            UnityEngine.Profiling.Profiler.BeginSample("BeginDrawPropertyTree");
#endif
...
#if UNITY_EDITOR
            UnityEngine.Profiling.Profiler.BeginSample("DrawPropertiesInTree");
#endif
...
#if UNITY_EDITOR
            UnityEngine.Profiling.Profiler.BeginSample("EndDrawPropertyTree");
#endif
    ...
    public sealed class PropertyTree<T> : PropertyTree
    {
...
#if UNITY_EDITOR
                if (string.IsNullOrEmpty(rootProperty.Name))
                {
                    UnityEngine.Profiling.Profiler.BeginSample("UpdateEmotyRootName");
                }
                else
                {
                    UnityEngine.Profiling.Profiler.BeginSample(rootProperty.Name);
                }
#endif
        ...
        {
#if UNITY_EDITOR
            UnityEngine.Profiling.Profiler.BeginSample(property.Path);
#endif
            ...
            try
            {
                ...
                //bool isShells = property.Path.EndsWith("Shells");
                ListDrawerSettingsAttribute listDrawerSettingsAttribute = null;
                foreach (var attribute in property.Info.Attributes)
                {
                    listDrawerSettingsAttribute = attribute as ListDrawerSettingsAttribute;
                    if (attribute != null)
                    {
                        break;
                    }
                ...
                for (int i = 0; i < property.Children.Count; i++)
                {
                    ...
                    if (listDrawerSettingsAttribute != null && listDrawerSettingsAttribute.NumberOfItemsPerPageHasValue
                                                            && listDrawerSettingsAttribute.NumberOfItemsPerPage > 0)
                    {
                        // 判断如果当前不符合每页的显示规则，则不渲染
                    }
//                     if (isShells)
//                     {
// #if UNITY_EDITOR
//                         UnityEngine.Profiling.Profiler.BeginSample(
//                             string.Format("UpdateProperty Child Index :{0} ", i));
// #endif
//                     }
//                     
