...
namespace Sirenix.Utilities.Editor
{
    ...
    internal class InstanceCreatorWindow : EditorWindow
    {
        ...
        private static bool CanCreateInstance(Type baseType, Type type)
        {
                ...
                rootNode = typeTrees[type] = new TypeTreeNode()
                {
                    ...
                    .Select(a => new TypeTreeNode()
                    {
                        ...
                        ChildNodes = AssemblyUtilities
                            ...
                            .Where(t => CanCreateInstance(type, t))
                            .Distinct()
        ...
        private class TypeTreeNode
        {
            ...
            private static Texture typeIcon = EditorGUIUtility.FindTexture("Prefab Icon");
            ...
            public void UpdateSearchTerm()
            {
                ...
                {
                    if (this.window.hasSearchTerm && FuzzySearch.Contains(ref this.searchString, ref this.window.searchTerm))
                    {
